...
const ManagePatientsPage: React.FC<ManagePatientsPageProps> = ({ setActiveView }) => {
  ...
  const handleDelete = async (patientId: string, patientName: string) => {
    if (window.confirm(`คุณแน่ใจหรือไม่ว่าต้องการลบข้อมูลผู้ป่วย "${patientName}"? การกระทำนี้ไม่สามารถย้อนกลับได้`)) {
      try {
        setLoading(true);
                ...
                <tr key={patient.id} className="hover:bg-gray-50/50">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center justify-center space-x-2">
                      ...
                      <button onClick={() => handleDelete(patient.id, patient.fullName)} className="p-2 rounded-full hover:bg-red-100 text-red-600" title="ลบ"><TrashIcon className="w-5 h-5" /></button>
