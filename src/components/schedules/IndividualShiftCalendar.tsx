...
const shiftStatusLabels: { [key in ShiftStatus]: string } = {
    [ShiftStatus.MORNING]: 'เวรเช้า',
    [ShiftStatus.AFTERNOON]: 'เวรบ่าย',
    [ShiftStatus.NIGHT]: 'เวรดึก',
    [ShiftStatus.DAY_OFF]: 'วันหยุด',
    [ShiftStatus.ON_LEAVE]: 'ลา',
};
...
const IndividualShiftCalendar: React.FC<IndividualShiftCalendarProps> = ({ drivers }) => {
    ...
    const weekEnd = currentWeek.add(6, 'day').format('D MMM BBBB');
                        ...
                        <tr className="bg-gray-50">
                            ...
                            {weekDays.map(day => (
                                <th key={day.format('YYYY-MM-DD')} className="p-3 text-center font-semibold text-sm text-gray-600 border">
                                    {day.locale('th').format('ddd')}<br />{day.format('D')}
                                </th>
                            ))}
                        </tr>
                    </thead>
                                        ...
                                        >
                                            {status && (
                                                <div className={`text-xs font-bold rounded p-1 ${shiftStatusStyles[status]}`}>
                                                    {shiftStatusLabels[status] || status}
                                                </div>
                                            )}
                                        </td>
                                    );
