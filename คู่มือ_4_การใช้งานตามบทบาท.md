...
- ✅ จัดการฐานข้อมูลโดยตรง (Reset, Seed, Raw SQL Query)
- ✅ ดู System Logs และ Debug Information (Real-time Socket Stats)
- ✅ ดูและแก้ไขการตั้งค่าระบบ (Config View)
...
**ฟีเจอร์:**
```typescript
// 1. System Overview (Tab: Overview)
- Server Status (Online/Offline)
- Database Actions (Factory Reset DB, Re-seed Users)
- Quick Links (System Logs, UX/UI Design System, API Docs)
- Real-time Console Output Log

// 2. SQL Playground (Tab: SQL Playground)
- Execute Raw SQL Queries (SELECT, INSERT, UPDATE, DELETE)
- View Query Results in Table Format
- Error Handling & Feedback

// 3. Socket Monitor (Tab: Socket Monitor)
- Real-time Socket.IO Server Stats
- Total Connected Clients
- WebSocket Connections Count
- Active Rooms List (User/Role based rooms)

// 4. System Config (Tab: System Config)
- View Runtime Environment Variables (Safe list)
- Debug Configuration Settings
```

### Use Cases

#### UC-DEV-01: ดู System Logs และสถานะ
```
1. เข้าสู่ Developer Dashboard
2. ดู Console Output เพื่อเช็คสถานะล่าสุด
3. คลิก "View System Logs" เพื่อดู log โดยละเอียดแบบย้อนหลัง
```

#### UC-DEV-02: ตรวจสอบและแก้ไขข้อมูลดิบ (SQL Playground)
```
1. เข้าสู่ Developer Dashboard -> Tab "SQL Playground"
2. พิมพ์คำสั่ง SQL เช่น `SELECT * FROM users WHERE email = 'test@example.com'`
3. คลิก "Execute Query"
4. ตรวจสอบผลลัพธ์ในตารางด้านล่าง
5. หากต้องการแก้ไข พิมพ์คำสั่ง UPDATE หรือ DELETE (ระวัง! การเปลี่ยนแปลงมีผลทันที)
```

#### UC-DEV-03: ตรวจสอบการเชื่อมต่อ Real-time (Socket Monitor)
```
1. เข้าสู่ Developer Dashboard -> Tab "Socket Monitor"
2. ดูจำนวน "WebSocket Conn" ว่ามีการเชื่อมต่อหรือไม่
3. ดู "Active Rooms" เพื่อเช็คว่า User ID หรือ Role ไหน Online อยู่บ้าง
4. คลิก "Refresh Stats" เพื่ออัปเดตข้อมูลล่าสุด
```

#### UC-DEV-04: ตรวจสอบ Config ของระบบ
```
1. เข้าสู่ Developer Dashboard -> Tab "System Config"
2. ดูค่า Environment Variables เช่น PORT, NODE_ENV, CORS_ORIGIN
3. ตรวจสอบว่าระบบรันด้วยค่า Config ที่ถูกต้องหรือไม่
```

#### UC-DEV-05: จัดการฐานข้อมูล (Reset/Seed)
```
1. เข้าสู่ Developer Dashboard -> Tab "Overview"
2. คลิก "Factory Reset Database" เพื่อล้างข้อมูลทั้งหมด (ระวัง!)
3. คลิก "Re-seed Test Users" เพื่อสร้างข้อมูลจำลองตั้งต้น
```
