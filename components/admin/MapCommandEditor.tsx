                ...
                <div className={`w-full border rounded-lg overflow-hidden relative z-0 ${isFullScreen ? 'flex-grow' : 'h-[600px]'}`}>
                    <MapContainer center={[19.9213, 99.2131]} zoom={13} style={{ height: '100%', width: '100%' }}>
                        ...
                        {showPatients && pendingRides.map(ride => {
                            // Note: In a real app, we'd need the patient's lat/lng here.
                            // For now, we'll use a placeholder or assume it's in the ride object.
                            if (!ride.latitude || !ride.longitude) return null;
                                ...
                                >
                                    <Popup>
                                        <div className="p-2 min-w-[150px]">
                                            <h3 className="font-bold text-sm">{ride.patient_name || ride.patientName}</h3>
                                            <p className="text-xs text-gray-600">{ride.trip_type || ride.tripType}</p>
                                            <p className="text-xs font-medium mt-1">สถานะ: <span className="text-red-600">{ride.status}</span></p>
                                            <p className="text-[10px] text-gray-400 mt-1">ID: {ride.id}</p>
                                        </div>
                                    </Popup>
                                </Marker>
                            );
