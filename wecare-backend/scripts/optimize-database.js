...
const sqliteDB_1 = require("../src/db/sqliteDB");
...
const createIndexes = () => {
    ...
    for (const index of indexes) {
        try {
            ...
            sqliteDB_1.sqliteDB.exec(`CREATE ${uniqueStr}INDEX IF NOT EXISTS ${index.name} ON ${index.table} (${columnsStr})`);
...
const analyzeTables = () => {
    ...
    for (const table of tables) {
        try {
            sqliteDB_1.sqliteDB.exec(`ANALYZE ${table}`);
...
const vacuumDatabase = () => {
    ...
    try {
        ...
        const beforeStats = sqliteDB_1.sqliteDB.prepare('PRAGMA page_count').get();
        const pageSize = sqliteDB_1.sqliteDB.prepare('PRAGMA page_size').get().page_size;
        const beforeSize = beforeStats.page_count * pageSize;
        console.log(`  Before: ${(beforeSize / 1024 / 1024).toFixed(2)} MB`);
        // Vacuum
        sqliteDB_1.sqliteDB.exec('VACUUM');
        // Get database size after
        const afterStats = sqliteDB_1.sqliteDB.prepare('PRAGMA page_count').get();
...
const optimizeSettings = () => {
    ...
    for (const setting of settings) {
        try {
            sqliteDB_1.sqliteDB.exec(`PRAGMA ${setting.pragma} = ${setting.value}`);
            const result = sqliteDB_1.sqliteDB.prepare(`PRAGMA ${setting.pragma}`).get();
...
const showStatistics = () => {
    ...
    for (const table of tables) {
        try {
            const count = sqliteDB_1.sqliteDB.get(`SELECT COUNT(*) as count FROM ${table}`);
            console.log(`  ${table.padEnd(20)} ${((count === null || count === void 0 ? void 0 : count.count) || 0).toLocaleString()} rows`);
        }
        catch (error) {
